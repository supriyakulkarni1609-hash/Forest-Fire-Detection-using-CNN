...
if __name__ == "__main__":
    ...
    (x_val, y_val), _ = extract_dataset(dataset_path, classes, percentage=0.8)

    val_gen = generate_from_paths_and_labels(
        x_val,
        y_val,
        batch_size=4,
        preprocessing=inception_preprocess_input
    )

    model = load_model(model_path)
    loss, acc = model.evaluate(val_gen, steps=max(1, len(x_val)//4))
